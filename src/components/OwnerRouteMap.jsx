...
function OwnerRouteMap({ destination, driverLocation }) {
  ...
  useEffect(() => {
    // Initialize map centered on parking slot (destination)
    if (!mapInstanceRef.current && mapRef.current) {
      ...
      // Add parking slot marker (red pin)
    ...
    console.log('Map initialized and marked as ready')
  ...
  useEffect(() => {
    console.log('OwnerRouteMap useEffect - driverLocation:', driverLocation, 'destination:', destination, 'mapInstanceRef.current:', mapInstanceRef.current, 'isMapReady:', isMapReadyRef.current)
    
    // If map is not ready yet, skip - it will re-run when driverLocation changes
    if (!mapInstanceRef.current || !isMapReadyRef.current) {
      console.log('Map not ready yet, skipping line draw')
      return
    }
    ...
    if (!driverLocation) {
      setLocationError('Driver location not available')
      setDistance(null)
      setDuration(null)

      // Center map on parking slot only
      mapInstanceRef.current.setView([destination.lat, destination.lng], 13)
      return
    }
    ...
    const updateRoute = () => {
      try {
        // Add/update driver location marker (blue dot)
        ...
        driverMarkerRef.current.bindPopup('Driver Location')
        ...
        // Remove existing line if any before adding new one
        if (routeLineRef.current) {
          mapInstanceRef.current.removeLayer(routeLineRef.current)
          routeLineRef.current = null
        }
        ...
        routeLineRef.current = L.polyline(latLngs, {
          ...
          zIndex: 500 // Ensure line appears above tile layer but below markers
        }).addTo(mapInstanceRef.current)
        
        // Bring the line to front to ensure visibility
        routeLineRef.current.bringToFront()
        
        console.log('Route line added to map:', routeLineRef.current)
    ...
    // Draw the route with a small delay to ensure map is fully rendered
    setTimeout(() => {
      if (mapInstanceRef.current) {
        updateRoute()
        
        // Fit bounds after line is drawn
  ...
  return (
    <div className="w-full">
      ...
      >
        <span>🎯</span> Recenter to Driver
      </button>
    </div>
  )
}

export default OwnerRouteMap
